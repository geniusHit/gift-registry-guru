...
async function handleFilterChange1() {
    ...
    if (currentShopPlan1 >= 2) {
        ...
        // const checkR = checkData(allHandle);
        // if (checkR) return; // if already valid, skip entire fetch process

        // // cache to prevent duplicate fetches
        // const cache = new Map();

        // // determine selector type once
        // const typeSelector =
        //     collectionBtnSetting.collectionType === "buttonType"
        //         ? ".wf-wishlist-collection-btn"
        //         : ".wf-wishlist-collection-icon";

        // // filter only handles that are not already in DOM
        // const handlesToFetch = allHandle.filter(handle => {
        //     const selector = `${typeSelector}[product-handle="${handle}"]`;
        //     return !document.querySelector(selector);
        // });

        // // function to fetch a single product safely with cache
        // async function fetchProduct(handle) {
        //     if (cache.has(handle)) return cache.get(handle);

        //     try {
        //         const response = await fetch(`${getDomain}products/${handle}.json`);
        ...
        // // limit concurrent fetches to avoid overloading network
        // async function runLimitedFetches(handles, limit = 5) {
        //     const results = [];
        //     const queue = [...handles];
        ...
        // // main logic inside your function
        // const products = await runLimitedFetches(handlesToFetch, 15); // you can increase limit if needed
