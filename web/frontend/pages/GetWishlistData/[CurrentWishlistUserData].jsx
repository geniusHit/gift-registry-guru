...
import {
    Page, IndexTable, Button, Modal, Text, Icon, Card, Grid, Select, Toast,
    DatePicker, Frame,
...
const GetWishlistData = () => {
    ...
    const wishlistDataTable = userList.map(({ id, variant_id, title, price, image, total_quantity, created_at, wishlist_name }, index) => (
        <IndexTable.Row id={index} key={index} position={index}>
            ...
            <IndexTable.Cell><Button onClick={() => deleteUserHandle(id, variant_id)}><Icon source={DeleteIcon} color="base" /></Button></IndexTable.Cell>
        </IndexTable.Row>
    ));
    ...
    const handleUrlChange = useCallback(
        (value) => {
            setRegistryData(prev => ({ ...prev, url_type: value }));
    ...
    const groupedOrders = orderedItems.reduce((acc, item) => {
    ...
    let ordersStartIndex = (parseInt(ordersPageNo) - 1) * parseInt(getItemRecordPerPage);
    let ordersEndIndex = ordersStartIndex + parseInt(getItemRecordPerPage);
    let hasOrdersNext = ordersEndIndex < orderKeys.length;
    let hasOrdersPrevious = parseInt(ordersPageNo) > 1;
                        ...
                        {groupedOrders[key].slice(0, 3).map(({ image }) => {
                            return (
                                <Grid.Cell columnSpan={{ xs: 4, sm: 4, md: 4, lg: 4, xl: 4 }}>
                                    <Thumbnail source={image} alt='image' size="small" />
                                </Grid.Cell>
                            )
                        })}
                    </Grid>
                            ...
                            <Grid.Cell columnSpan={{ xs: 6, sm: 6, md: 6, lg: 6, xl: 6 }}>
                                <Grid>
                                    <Grid.Cell columnSpan={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }}>
                                        <Text variant="headingSm" as="b">
                                            {title}
                                        </Text>
                                    </Grid.Cell>
                                </Grid>
                                <Grid>
                                    <Grid.Cell columnSpan={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }}>{parseFloat(parseInt(price) / parseInt(quantity)).toFixed(1)} ✕ {quantity}</Grid.Cell>
                                </Grid>
                            </Grid.Cell>
                        </Grid>
                    </Box>
...
// import useApi from '../../hooks/useApi';
// import {
//     Page, LegacyCard, IndexTable, Button, Modal, Text, Icon, Card, Grid, Select, Toast,
//     DatePicker, Frame,
//     TextField,
...
// import { ArrowLeftMinor, DeleteIcon, ClipboardMinor } from '@shopify/polaris-icons';
// import Swal from 'sweetalert2';
// import loaderGif from "../loaderGreen.gif"
// import Footer from '../Footer';
// import collectionCopyIcon from '../../assets/copy-icon.svg'
